...
1. Supprime les anciens fichiers (ou crée une nouvelle branche)
1. Upload TOUS les fichiers ci-dessus EN RESPECTANT LA STRUCTURE des dossiers
1. Commit & push
...
1. **Test chat Dylan** : Pose une question simple (“voyant moteur Renault Clio”)
1. **Test VIN** : Décode un VIN (gratuit)
1. **Test paiement Stripe** : Pack 1€ avec ta vraie carte
- Si ça marche, tu peux te rembourser sur dashboard.stripe.com
1. **Test code propriétaire** : Code `LOIC2024` pour crédits illimités
1. **Test photo** : Upload une photo de pièce
1. **Test urgence** : Bouton “C’est grave docteur”
...
1. Frontend appelle `stripe-checkout` (création de session)
1. User est redirigé vers la page de paiement Stripe sécurisée
1. User paie
1. Stripe envoie une confirmation au `stripe-webhook`
1. Le webhook :
- Met à jour les crédits dans Supabase
- Envoie un email de confirmation via Resend
1. User est redirigé vers ton site avec un message de succès
